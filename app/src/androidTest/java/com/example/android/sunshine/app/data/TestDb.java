...
//        insertLocation(db,northPoleLocationValues);
    ...
    public void testWeatherTable() {
        ...
        assertEquals(true, WeatherDb.isOpen());
        ...
        // Insert ContentValues into database and get a row ID back
        long weatherRowID = WeatherDb.insert(WeatherContract.WeatherEntry.TABLE_NAME,null,weatherValues);
        assertTrue("Weather row insert failed", weatherRowID != -1);

        // Query the database and receive a Cursor back
        Cursor weatherCusor = WeatherDb.query(WeatherContract.WeatherEntry.TABLE_NAME,null,null,null,null,null,null);

        // Move the cursor to a valid database row
        weatherCusor.moveToFirst();

        // Validate data in resulting Cursor with the original ContentValues
        // (you can use the validateCurrentRecord function in TestUtilities to validate the
        // query if you like)
        validateCurrentRecord("Values didn't match up", weatherCusor,weatherValues);
